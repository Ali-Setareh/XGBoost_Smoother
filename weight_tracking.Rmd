...
# Plot training data with true function line
...
dtrain = xgb.DMatrix(data = as.matrix(train_data$X), label = train_data$Y)
dtest = xgb.DMatrix(data = as.matrix(test_data$X), label = test _data$Y)

# Define model parameters
params = list(
  booster = "gbtree",
  objective = "reg:squarederror",
  eta = 0.1, # Equivalent to learning_rate
  max_depth = 1, # Need to specify a value as XGBoost requires a numerical value
...
```
```{r}
smoothers = get_xgboost_weights(model,dtrain,)
```
...
data <- tibble(
  value = c(y_true[train_indices], y_hat_xg, Y[train_indices]),
  X_grid = rep(X[train_indices], 3),
  Method = factor(c(rep("True Function", length(X[train_indices])), rep("XGBoost", length(X[train_indices])), rep("Observed", length(X[train_indices]))))
)
...
weights_at_zero_XG = smoothers$S_train[which(X[train_indices]==0),]

```

```{r}
length(weights_at_zero_XG)
```

```{r fig.width=4, fig.height=2}
# Create a tibble with the X and Y values
data <- tibble(X = X[train_indices], Y = weights_at_zero_XG)
