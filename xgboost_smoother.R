...
create_S_from_tree = function(tree_indices,n, lambda ){
...
create_S_from_single_boosted_tree = function(tree_indices,S_gb_prev,num_samples,lambda){
...
create_S_from_gbtregressor = function(model,leaf_indices,output_dir = NULL, save_output = FALSE, compact = FALSE){
  ...
  for (col in 1:ncol(leaf_indices)) {
    ...
    S = create_S_from_single_boosted_tree(current_tree,if(col == 1) NULL else S_curr ,n, lambda)
    S_curr = S_curr + lr * S
